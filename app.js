document.addEventListener('DOMContentLoaded', () => {
    try {
        ...
        let pdfResults = {}; // Oggetto per PDF e Dettagli
        ...
        // --- 2. LOGICA DI CALCOLO (Il "Cervello") ---

        /**
         * Esegue tutti i calcoli finanziari. Non legge/scrive sul DOM.
         * @param {object} inputs - Un oggetto contenente tutti i valori di input parsati.
         * @returns {object} Un oggetto contenente tutti i risultati dei calcoli.
         */
        function calculateFinancials(inputs) {
            ...
            const produzioneAnnua = inputs.potenzaInstallata * inputs.produzione_per_kWp;

            // 2. Calcolo PNRR
            const potenzaPerCalcoloPNRR = Math.min(inputs.potenzaInstallata, inputs.potenzaInverterInstallata);
            ...
            // 3. Calcolo Flussi Anno 1
            const kwhAutoconsumati = Math.min(produzioneAnnua * inputs.autoconsumoPerc, consumoTotale);
            const kwhImmessi = produzioneAnnua - kwhAutoconsumati;
            const kwhCondivisi = kwhImmessi * inputs.percEnergiaCondivisa;

            const risparmioAutoconsumo = kwhAutoconsumati * inputs.costoQuotaEnergia;
            ...
            return {
                ...
                importoIncentivoPNRR,
                costoRealeCliente,
                potenzaPerCalcoloPNRR, // Aggiunto per UI
                massimale_kwp_dinamico, // Aggiunto per UI
                
                // Dati Benefici Anno 1
                risparmioAutoconsumo,
                guadagnoImmissione,
                totaleBeneficiPrimoAnno,
                ...
                kwhImmessi,
                kwhCondivisi,
                prezzoRID: inputs.prezzoRID,
                tariffaCER: inputs.tariffaCER,
                percEnergiaCondivisa: inputs.percEnergiaCondivisa,
                produzione_per_kWp: inputs.produzione_per_kWp,
        ...
        // --- 3. LOGICA DI INTERFACCIA (Le "Mani") ---

        /**
         * Legge tutti i valori dai campi input e li restituisce come oggetto.
         * @returns {object} Oggetto con tutti i valori di input.
         */
        function getInputs() {
            return {
                ...
                autoconsumoPerc: (parseFloat(inAutoconsumoPerc.value) / 100) || 0,
                produzione_per_kWp: parseFloat(inProduzioneKwhKwp.value) || 0,
                potenzaInstallata: parseFloat(inPotenzaInstallata.value) || 0,
                potenzaInverterInstallata: parseFloat(inPotenzaInverter.value) || 0,
        ...
        /**
         * Aggiorna tutti gli elementi dell'interfaccia utente con i risultati.
         * @param {object} results - L'oggetto restituito da calculateFinancials.
         */
        function updateUI(results) {
            ...
            outPanoProduzione.textContent = `${results.produzioneAnnua.toFixed(0)} kWh`;
        ...
        /**
         * Popola la tabella del piano di rientro.
         * @param {Array} plan - L'array paybackPlan dai risultati.
         */
        ...
        /**
         * Disegna o aggiorna il grafico del flusso di cassa.
         * @param {object} chartData - L'oggetto chartData dai risultati.
         */
        ...
        /**
         * Valida tutti gli input, aggiungendo/rimuovendo classi CSS.
         * @returns {boolean} True se tutti gli input sono validi.
         */
        ...
        /**
         * Aggiorna la sezione "Progetto Consigliato" in base agli input.
         */
        ...
        /**
         * Popola il pannello "Dettaglio Calcoli" (usa lo stato globale pdfResults).
        ...
        function populateCalculationDetails() {
            try {
                ...
                // (Validazione dati omessa per brevità, è identica a prima)
        ...
        /**
         * Genera ed esporta il report PDF (usa lo stato globale pdfResults).
         */
        async function exportToPdf() {
            ...
            try {
                ...
                const colorBgSummaryRed = '#fef2f2'; 
                ...
                const drawExplanationBox = (rawText) => {
                    const textPadding = 4;
                    const textMargin = margin + textPadding + 3;
                    const textWidth = usableWidth - (textPadding * 2) - 6; 
                ...
                pdf.setFillColor(colorBgSummaryRed);
                pdf.setDrawColor(colorBorderSummaryRed);
        ...
        /**
         * Salva lo stato corrente dei form nel localStorage.
         */
        function saveState() {
            const state = {
        ...
        /**
         * Carica lo stato dal localStorage o imposta i valori di default.
         */
        function loadState() {
        ...
        /**
         * Gestore principale per il click su "Calcola".
         * Coordina validazione, calcolo e aggiornamento UI.
         */
        function handleCalculateClick() {
            ...
            try {
                ...
                // 3. Salva Stato
                pdfResults = results; // Salva per PDF e Dettagli
                calcoliEseguiti = true;

                // 4. Aggiorna UI
                updateUI(results);

                // 5. Aggiorna dettagli (se aperti)
        ...
        toggleDetailsBtn.addEventListener('click', () => {
            ...
            if (isHidden) {
                detailsContent.innerHTML = populateCalculationDetails(); // Popola qui
